...
### Added
- Schema generation now honours `System.ComponentModel.DataAnnotations` validation attributes, projecting them onto the equivalent JSON Schema facets: `[Required]` (including on reference-typed properties such as `string` and nested models), `[StringLength]`/`[MinLength]`/`[MaxLength]` (`minLength`/`maxLength` for strings, `minItems`/`maxItems` for collections), `[Range]` (`minimum`/`maximum`), `[RegularExpression]` (`pattern`), `[EmailAddress]`/`[Url]` (`format`), `[ReadOnly(true)]` (`readOnly`), and `[DefaultValue]` (`default`).
...
### Fixed
- `DefaultSchemaGenerator` now marks a property `required` when it carries `[Required]`. Previously only non-nullable value types were treated as required, so a `[Required]` reference-typed property (e.g. a `string` or nested model) was silently omitted from the schema's `required` array.
